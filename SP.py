...
def sp_distillation(outputs, teacher_outputs):
    # 计算相似度矩阵
    sim = F.cosine_similarity(outputs, teacher_outputs, dim=1)
    # 计算相似度损失
...
#%%
    ...
    for epoch in range(201):
        ...
        for step, (batch_x, batch_y) in enumerate(loader):
            ...
            loss_sp = sp_distillation(student_outputs,teacher_outputs)
            loss = 0.5*loss_ce+0.5*loss_sp
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
