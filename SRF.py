...
from unet_cfakd import dwt, idwt, FrequencyPrompt, PositionAwareRelationalLoss
from unet_cfakd import UNetCustom as UNet_s
...
model_path = './train_unet_teacher_ce/epoch40_0.9816.pth'
...
#%%
# 添加Frequency Prompt和Position-aware Relational Loss
frequency_prompt = FrequencyPrompt(channels=ndf*2, num_prompts=10).cuda()
position_aware_loss = PositionAwareRelationalLoss().cuda()
...
#%%
    model_dir = 'train_unet_kd_cfakde2'
    ...
    for epoch in range(201):
        ...
        for step, (batch_x, batch_y) in enumerate(loader):
            ...
            # 进行频域变换
            teacher_dwt, teacher_cd = dwt(teacher_outputs)
            student_dwt, student_cd = dwt(student_outputs)

            # 应用频率提示
            teacher_dwt_masked = frequency_prompt(teacher_dwt)
            student_dwt_masked = frequency_prompt(student_dwt)

            loss_ce = distill_criterion(student_outputs, teacher_outputs, batch_y)
            loss_crd = CRD(teacher_outputs,student_outputs,5)
            loss_pa = position_aware_loss(teacher_dwt_masked, student_dwt_masked)
            loss = 0.3*loss_ce+ 0.2*loss_crd + 0.5*loss_pa
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
