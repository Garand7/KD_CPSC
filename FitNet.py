...
# 假设 enc1 是第一个感兴趣的层，enc2 是第二个
teacher_model.enc1.register_forward_hook(get_features('teacher_enc1'))
teacher_model.enc2.register_forward_hook(get_features('teacher_enc2'))
student_model.enc1.register_forward_hook(get_features('student_enc1'))
student_model.enc2.register_forward_hook(get_features('student_enc2'))
#%%
# 加载教师网络权重
...
#%%
    ...
    for epoch in range(201):
        ...
        for step, (batch_x, batch_y) in enumerate(loader):
            ...
            # 使用保存的特征计算额外的损失
            loss_fitnet = nn.MSELoss()(features['student_enc1'], features['teacher_enc1'])

            loss = 0.5 * loss_ce + 0.5 * loss_fitnet
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
