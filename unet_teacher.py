...
class UNetCustom(nn.Module):
    def __init__(self, input_nc, ndf):
        ...
        self.enc4 = nn.Sequential(
            nn.Conv2d(ndf * 4, ndf * 8, kernel_size=4, stride=2, padding=2),
        ...
        self.dec1 = nn.Sequential(
            nn.ConvTranspose2d(ndf * 8, ndf * 4, kernel_size=4, stride=2, padding=2, output_padding=1),
            nn.BatchNorm2d(ndf * 4),
            nn.LeakyReLU(0.1, inplace=True)
        )
    ...
    def forward(self, x):
        ...
        # 插值调整尺寸
        d3_resized = F.interpolate(d3, size=e1.size()[2:], mode='bilinear', align_corners=False)
