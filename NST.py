...
class NSTLoss(nn.Module):
    """Neuron Selectivity Transfer Loss"""
    def __init__(self, temperature=10.0):
        super(NSTLoss, self).__init__()
        self.temperature = temperature

    def forward(self, student_outputs, teacher_outputs):
        # Compute the Neuron Selectivity Transfer loss using the polynomial kernel
        nst_loss = self.poly_kernel(student_outputs, teacher_outputs.detach())

        return nst_loss

    def poly_kernel(self, s_out, t_out):
        # Compute the polynomial kernel between the student and teacher outputs
        M = torch.dot(s_out.view(-1), s_out.view(-1))  # scalar
        N = torch.dot(t_out.view(-1), t_out.view(-1))  # scalar
        P = torch.dot(s_out.view(-1), t_out.view(-1))  # scalar
        K = ((self.temperature * P + 1) ** 2) / ((self.temperature * M + 1) * (self.temperature * N + 1))
...
#%%
    ...
    for epoch in range(201):
        ...
        for step, (batch_x, batch_y) in enumerate(loader):
            ...
            nstLOSS = NSTLoss()
            loss_nst = nstLOSS(student_outputs, teacher_outputs)
            loss = 0.5*loss_ce+0.5*loss_nst
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
