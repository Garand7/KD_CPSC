...
def rkd_distance(student_features, teacher_features, beta):
    # Assuming student_features and teacher_features are 2D: (batch_size, features)
    student_dists = torch.cdist(student_features, student_features, p=2)
    teacher_dists = torch.cdist(teacher_features, teacher_features, p=2)

    loss = F.smooth_l1_loss(student_dists, teacher_dists, reduction='mean')
    return loss


class RKDLoss(nn.Module):
    def __init__(self, alpha=0.6, beta=150):
    ...
    def forward(self, student_outputs, teacher_outputs):
        # Normalize features
        student_features = F.normalize(student_outputs, p=2, dim=1)
        teacher_features = F.normalize(teacher_outputs, p=2, dim=1)

        # Compute RKD distance based loss
        dist_loss = rkd_distance(student_features, teacher_features, self.beta)
...
distill_criterion = DistillationLoss(alpha=0.5, temperature=5)
...
#%%
    ...
    rkd_loss_func = RKDLoss(alpha=alpha, beta=150)
    ...
    for epoch in range(201):
        ...
        for batch_x, batch_y in loader:
            ...
            distillation_loss = distill_criterion(student_outputs, teacher_outputs, batch_y)
            relational_loss = rkd_loss_func(student_outputs, teacher_outputs)

            loss = 0.5 * distillation_loss + 0.5 * relational_loss  # Adjust weighting as necessary
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
